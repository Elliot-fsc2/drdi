...
new
    ...
    public function mount(): void
    {
        ...
        if ($section) {
            $group = Cache::flexible("student_{$userId}_group", [300, 900], function () use ($section, $user) {
            ...
            if ($group) {
                $this->proposals = Cache::flexible("student_{$userId}_proposals", [300, 900], function () use ($group) {
                    return Proposal::with('submittedBy')
                ...
                $this->consultations = Cache::flexible("student_{$userId}_consultations", [300, 900], function () use ($group) {
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 lg:py-12">
        ...
        <div class="mb-8 sm:mb-10">
            <div class="inline-flex items-center gap-2 rounded-full border px-4 py-1.5 mb-5"
                ...
                <span
                    style="font-family: 'JetBrains Mono', monospace; font-size: 11px; letter-spacing: 0.14em; color: #0052FF; text-transform: uppercase">Student
                    Portal</span>
            </div>
            <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
                <div>
                    ...
                    <p class="mt-2 text-sm" style="color: #64748B">Track your proposals, groups, and consultations all
                        in one place.</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-5">
                <livewire:student-announcements lazy />
            </div>
            ...
            <div class="space-y-5 lg:sticky lg:top-8 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto">
                @if ($section)
                    ...
                    <div class="rounded-2xl border p-4 transition-all duration-200 hover:-translate-y-px hover:shadow-lg"
                        ...
                        <p
                            style="font-family: 'JetBrains Mono', monospace; font-size: 10px; letter-spacing: 0.1em; color: #94A3B8; text-transform: uppercase">
                            Instructor</p>
                        <p class="text-xl font-bold mt-1" style="color: #0F172A">
                            {{ $section->instructor?->full_name ?? 'N/A' }}</p>
                    ...
                    <div class="rounded-2xl border overflow-hidden"
                        ...
                        <div class="px-5 py-4" style="border-bottom: 1px solid #F1F5F9; background: #FAFAFA">
                            <span
                                style="font-family: 'JetBrains Mono', monospace; font-size: 10px; letter-spacing: 0.12em; color: #94A3B8; text-transform: uppercase">My
                                Proposals</span>
                        </div>
                    ...
                    <div class="rounded-2xl border overflow-hidden"
                        ...
                        <div class="px-5 py-4" style="border-bottom: 1px solid #F1F5F9; background: #FAFAFA">
                            <span
                                style="font-family: 'JetBrains Mono', monospace; font-size: 10px; letter-spacing: 0.12em; color: #94A3B8; text-transform: uppercase">Consultation
                                Schedule</span>
                        </div>
                                ...
                                <div class="flex gap-3 items-start">
                                    <div class="shrink-0 w-12 rounded-xl text-center py-2"
                                        ...
                                        <div class="text-[10px] font-bold uppercase" style="color: #0052FF">
                                            {{ $c->scheduled_at?->format('M') }}</div>
                                        <div class="text-lg font-bold leading-none" style="color: #0052FF">
                                            {{ $c->scheduled_at?->format('d') }}</div>
                                    </div>
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold" style="color: #0F172A">
                                            {{ $section->instructor?->full_name ?? 'No instructor assigned' }}</p>
                ...
                @else
                    <div class="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed py-20 px-8 text-center"
                        ...
                        <h3 class="mb-2"
                            style="font-family: 'Calistoga', Georgia, serif; font-size: 1.4rem; color: #0F172A">No active
                            section found</h3>
                        <p class="text-sm max-w-xs" style="color: #64748B; line-height: 1.6">Please contact your
                            administrator to be assigned to an active section.</p>
                    </div>
                @endif
            </div>
        </div>

...
<div class="p-4 sm:p-6 lg:p-8 min-h-screen bg-slate-50">
    <div class="max-w-4xl mx-auto">
        ...
        @else
            <div class="border border-slate-200 bg-white rounded-xl shadow-sm overflow-hidden divide-y divide-slate-100">
                @foreach($this->consultations as $consultation)
                    <div
                        class="group hover:bg-slate-50 transition-colors cursor-pointer p-4 sm:p-5 flex flex-col sm:flex-row gap-4 sm:items-center relative"
                        wire:key="{{ $consultation->id }}"
                        wire:click="mountAction('viewConsultation', { consultation: '{{ $consultation->id }}' })">
                        
                        <div class="absolute inset-y-0 left-0 w-1 {{ $consultation->status === 'completed' || $consultation->status === 'approved' ? 'bg-emerald-400' : 'bg-transparent group-hover:bg-slate-300' }} transition-colors"></div>
                        
                        <div class="sm:w-36 shrink-0">
                            @if($consultation->scheduled_at)
                                <div class="text-xs text-slate-500 font-medium tracking-wide">{{ $consultation->scheduled_at->format('M d, Y') }}</div>
                                <div class="text-sm font-semibold text-slate-900 mt-0.5">{{ $consultation->scheduled_at->format('g:i A') }}</div>
                            @else
                                <span class="inline-flex rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">Unscheduled</span>
                            @endif
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <span class="inline-flex items-center rounded px-2 py-0.5 text-xs font-medium ring-1 ring-inset {{ $this->getStatusColorClass($consultation->status) }}">

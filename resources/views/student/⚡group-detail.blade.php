    ...
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        @if (!$this->group())
            <div class="bg-white border border-slate-200 rounded-lg p-12 sm:p-20 text-center shadow-sm">
                ...
                <p class="text-slate-500 text-sm sm:text-base mt-1">You are not assigned to any active group for the
                    current semester.</p>
            </div>
        @else
                ...
                <div class="relative overflow-hidden bg-white border border-slate-200 rounded-xl shadow-sm">
                    ...
                    <div class="px-6 py-5 sm:p-6 bg-linear-to-r from-green-50/50 to-white">
                        <div class="flex items-center gap-2 mb-3">
                            <h3 class="font-bold text-slate-900 text-sm uppercase tracking-wider">Final Approved Title
                            </h3>
                        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="divide-y divide-slate-100">
                    @foreach ($this->group()->members->sortByDesc(fn($m) => $m->id === $this->group()->leader_id) as $member)
                        <div
                            class="flex items-center justify-between px-6 py-4 hover:bg-slate-50 transition-colors duration-150">
                            <div class="flex items-center gap-4">
                                <div
                                    class="h-10 w-10 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-600 text-sm font-bold shadow-sm">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span
                                        class="text-xs text-slate-500 mt-0.5">{{ $member->student_number ?? 'Student' }}</span>
                                </div>
                            </div>
                            ...
                            @if ($member->id === $this->group()->leader_id)
                                <div class="flex mt-2 sm:mt-0">
                                    <span
                                        class="inline-flex items-center gap-1 rounded-md bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700 ring-1 ring-inset ring-blue-600/20">
                            ...
                            @elseif($member->id === auth()->user()->profileable->id)
                                <div class="flex mt-2 sm:mt-0">
                                    <span
                                        class="inline-flex items-center rounded-md bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600 ring-1 ring-inset ring-slate-500/10">
                                        You
                                    </span>
                                </div>
                            @endif
            ...
            <div class="flex items-center justify-center mt-6">
                <p class="text-xs text-slate-400">If you need to update group information, please contact your
                    instructor.</p>
            </div>
        @endif
    </div>

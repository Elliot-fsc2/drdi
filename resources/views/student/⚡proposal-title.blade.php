...
<div class="p-3 sm:p-4 lg:p-6 bg-slate-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
        ...
        @if ($this->group() && $this->group()->finalTitle)
            <div
                class="mb-6 bg-linear-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-4 sm:p-5 shadow-sm">
                <div class="flex items-start gap-4">
                    <div class="flex-1">
                        <h2 class="text-sm font-bold text-green-800 uppercase tracking-widest mb-1">Final Approved Title
                        </h2>
                        <h3 class="text-lg sm:text-xl font-bold text-slate-900 leading-tight">
                            {{ $this->group()->finalTitle->title }}</h3>
                        <p class="text-sm text-slate-600 mt-2 line-clamp-3">
                            {{ $this->group()->finalTitle->description }}</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        @else
            @island(name: 'proposals-list')
                ...
                <x-filament::pagination :paginator="$this->proposals" class="mt-5 mx-auto" />
            @endisland
        @endif
    </div>
    <x-filament-actions::modals />

    <x-filament::modal id="propose_title" :close-button="false" width="xl">
        {{-- Modal content --}}
        <x-slot name="heading">
            Propose New Title
        </x-slot>

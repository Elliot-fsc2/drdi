        ...
        <aside x-show="mobileMenuOpen" x-transition:enter="transition ease-in-out duration-300 transform"
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-6">
                <a href="{{ route('instructor.home') }}" wire:navigate @class([
                    ...
                    'bg-white/15 text-white border border-white/25' => request()->routeIs('instructor.home'),
                    'text-blue-200 hover:bg-blue-700/50 hover:text-white' => !request()->routeIs('instructor.home'),
                ])>
                ...
                <a href="{{ route('instructor.classes') }}" wire:navigate @class([
                    'flex items-center gap-3 rounded-xl px-4 py-2 transition-all',
                    'bg-white/15 text-white border border-white/25' => request()->routeIs('instructor.classes*'),
                    'text-blue-200 hover:bg-blue-700/50 hover:text-white' => !request()->routeIs('instructor.classes*'),
                ])>
                ...
                <a href="{{ route('instructor.groups') }}" wire:navigate @class([
                    'flex items-center gap-3 rounded-xl px-4 py-2 transition-all',
                    'bg-white/15 text-white border border-white/25' => request()->routeIs('instructor.groups*'),
                    'text-blue-200 hover:bg-blue-700/50 hover:text-white' => !request()->routeIs('instructor.groups*'),
                ])>
                ...
                <a href="{{ route('instructor.library-submissions') }}" wire:navigate @class([
                    ...
                    'text-blue-200 hover:bg-blue-700/50 hover:text-white' => !request()->routeIs('instructor.library-submissions*'),
                ])>
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto py-6" :class="sidebarOpen ? 'px-4' : 'px-3'">
                ...
                <a href="{{ route('instructor.library-submissions') }}" wire:navigate
                    :class="sidebarOpen ? 'px-4' : 'px-3 justify-center'" @class([
                        ...
                        'bg-white/15 text-white border border-white/25' => request()->routeIs('instructor.library-submissions*'),
                        'text-blue-200 hover:bg-blue-700/50 hover:text-white' => !request()->routeIs('instructor.library-submissions*'),
                    ]) :title="!sidebarOpen ? 'Library Submissions' : null">
                    <x-heroicon-o-archive-box-arrow-down class="h-6 w-6 shrink-0" />
                    <span x-show="sidebarOpen" x-transition class="whitespace-nowrap font-medium">Library Submissions</span>
                </a>
                    ...
                    <div class="flex items-center justify-between gap-4 py-3">
                        ...
                        <div class="flex min-w-0 items-center gap-3">
                            <div
                                ...
                                <span class="truncate">{{ auth()->user()->profileable?->role ?? 'Instructor' }}</span>
                            </div>
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-100" x-cloak
                                    ...
                                    <div class="border-b border-slate-100 bg-slate-50/80 px-4 py-3">
                                        ...
                                        <div class="mt-2 inline-flex items-center rounded-full bg-cyan-100 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-cyan-800">
                                            {{ auth()->user()->profileable?->role ?? 'Instructor' }}
                                        </div>
                                    </div>
